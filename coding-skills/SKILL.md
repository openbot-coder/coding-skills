...
所有开发任务首先需要评估项目规模，再路由到对应的开发流程。

| 规模 | 功能点 | 文档要求 | 开发流程 |
|------|--------|----------|----------|
| **脚本类** | ≤ 50 FP | 无需文档 | 直接编写代码 + 单元测试（覆盖率 100%） |
| **中小型** | < 1000 FP | 1 份 PRD + 1 份设计 | 完整 8 阶段流程 |
| **大型** | ≥ 1000 FP | 按模块拆分 | 按模块独立开发，统一管理 |
...
digraph coding_lifecycle {
    ...
    "评估项目规模" [shape=diamond];
    "脚本类（≤50 FP）" [shape=box, style=filled, fillcolor=lightgreen];
    "中小型（<1000 FP）" [shape=box];
    "大型（≥1000 FP）" [shape=box];

    "收到开发任务" -> "评估项目规模";
    "评估项目规模" -> "脚本类（≤50 FP）" [label="功能点≤50\n单一文件可完成"];
    "评估项目规模" -> "中小型（<1000 FP）" [label="功能点<1000"];
    "评估项目规模" -> "大型（≥1000 FP）" [label="功能点≥1000\n需模块拆分"];

    "脚本类（≤50 FP）" -> "script-development" [style=dashed];
    ...
    "阶段1：构思" -> "阶段2：PRD与设计" [label="构思完成"];
    "阶段2：PRD与设计" -> "阶段3：实施计划" [label="设计已批准"];
    "阶段3：实施计划" -> "阶段4：执行" [label="计划已存在"];
    "阶段4：执行" -> "阶段5：调试" [label="bug / 错误"];
    "阶段5：调试" -> "阶段6：审查" [label="调试完成"];
    "阶段6：审查" -> "阶段7：验证" [label="审查通过"];
    "阶段7：验证" -> "阶段8：收尾" [label="验证通过"];
...
| 你听到... | 规模 | 流程 |
|-----------|------|------|
| "写个脚本处理 X" / "小工具" | 脚本类 | script-development |
| "做个功能" / "实现 XX" (功能点 < 1000) | 中小型 | 阶段 1-8 |
| "大项目" / "系统重构" (功能点 ≥ 1000) | 大型 | 按模块执行阶段 1-8 |
