...
不稳定的测试通常通过任意延迟来猜测时序。这会导致竞态条件——测试在快速机器上通过，但在高负载或 CI 环境中失败。
...
参见本目录下的 `condition-based-waiting-example.ts`，其中包含来自实际调试会话的完整实现和领域专用辅助函数（`waitForEvent`、`waitForEventCount`、`waitForEventMatch`）。
...
```typescript
// 工具每100ms执行一次 — 需要2次执行来验证部分输出
await waitForEvent(manager, 'TOOL_STARTED'); // 首先：等待条件
await new Promise(r => setTimeout(r, 200));   // 然后：等待定时行为
// 200ms = 以100ms间隔执行2次 — 有文档说明且有理有据
```
