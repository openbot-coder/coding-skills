...
**归档时创建 PR：** 归档完成后，必须创建 Pull Request 将 `develop` 分支合并到 `main` 分支。
...
> **任务拆解**：参考 [task-breakdown 子技能](./task-breakdown/SKILL.md)

**概述：**

- 将已批准的设计文档拆解为可执行的任务清单
- 专门的任务拆解技能，避免与 Agent 的通用 plan 技能冲突
- 每个任务 10\~20 个功能点，明确依赖关系和验证方式
...
2. 脚本将：
   - 提交 Git 到 `develop` 分支（`git add . && git commit -m "chore: 完成变更 <name>"`）
   - 推送到远程仓库
